    ...
    for (int i = 0; i < src.rows; ++i)
        for (int j = 0; j < src.cols; ++j) {
            ...
            if (!(i < r1 || i >= src.rows - r1 || j < r2 || j >= src.cols - r2)) {
                ...
                if (true) {  // Method 1: Replicate the pixels near the edge.
                    for (int m = -r1; m <= r1; m++)
                        for (int n = -r2; n <= r2; n++) {
                            int ii = i + m, jj = j + n;
                            ii = (ii < 0 ? 0 : ii);
...
cv::Mat gaussion(const cv::Mat &src, int ksize) {
    ...
    if (blurred.depth() != src.depth())  // conv2D result is double. Need to convert back to uint8
 ...
 * Procedures:
 *      1. Compute image gradient Ix, Iy using sobel.
...
cv::Mat1b canny(const cv::Mat1b &src, const double lb, const double ub) {
    ...
    constexpr double SCALE_GRADIENT_MAG = 16.0;

    const int r1 = 1;       // radius of the gaussian filter.
    const int r2 = 1;       // radius of the sobel kernel. I only implemented the ksize=3.
    const int r = r1 + r2;  // Pixels near the image edge within this radius are not processed, which needs improvement!
    ...
    if (1) {  // This is important! You can set this to false and try.
